...
int
CaloAna::process_event( PHCompositeNode* topNode )
{
...
int
CaloAna::process_g4hits( PHCompositeNode* topNode )
{
...
int
CaloAna::process_g4cells( PHCompositeNode* topNode )
{
  ostringstream nodename;
  ...
  geonodename.str("");
  geonodename << "CYLINDERCELLGEOM_" << detector;
  ...
  if (cells)
    {
      ...
      // this returns iterators to the beginning and the end of the cell map
...
int
CaloAna::process_towers( PHCompositeNode* topNode )
{
  ostringstream nodename;
  ostringstream geonodename;
...
int
CaloAna::End(PHCompositeNode * topNode)
{
  outfile->cd();
  g4hitntuple->Write();
  g4cellntuple->Write();
