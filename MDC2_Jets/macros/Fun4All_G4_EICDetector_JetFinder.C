...
R__LOAD_LIBRARY(libjetfinder.so)

int Fun4All_G4_EICDetector_JetFinder(
    const int nEvents = 1,
...
//  Enable::DSTREADER = true;
  ...
  JetFinder *jetFinder = new JetFinder("jetFinder", outputroot + "_jetFinder.root");
  jetFinder->setMinJetPt(3.);
  jetFinder->Verbosity(0);
  jetFinder->analyzeTracks(true);
  jetFinder->analyzeClusters(true);
  jetFinder->analyzeJets(true);
  jetFinder->analyzeTruth(false);
  se->registerSubsystem(jetFinder);
