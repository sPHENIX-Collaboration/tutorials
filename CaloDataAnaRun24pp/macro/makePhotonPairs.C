...
void makePhotonPairs(const char* fin = "commissioning.root")
{
  SetsPhenixStyle();
  ...
  TH1F *mass = new TH1F("invMass","invMass",100,0,1);
  ...
  for(int i = 0; i < T -> GetEntries(); i++)
      ...
      for(int clus1 = 0; clus1 < clusterE->size(); clus1++)
	{
	  ...
	  if(clusterE -> at(clus1) < minClusE) continue;
	   ...
	   TLorentzVector photon1;photon1.SetPtEtaPhiE(clusterPt -> at(clus1), clusterEta -> at(clus1), clusterPhi -> at(clus1), clusterE -> at(clus1));
	  ...
	  for(int clus2 = 0; clus2 < clusterE->size(); clus2++)
	    {
	      ...
	      TLorentzVector photon2;photon2.SetPtEtaPhiE(clusterPt -> at(clus2), clusterEta -> at(clus2), clusterPhi -> at(clus2), clusterE -> at(clus2));
